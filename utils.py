...
def message_to_tensor_exp(message, sigma, delta):
    secret_tensor = torch.zeros(size=(message.shape[0], message.shape[1] // sigma))
    step = 2 / 2 ** sigma
    step1 = 2 / exp(sigma)
    random_interval_size = step1 * delta
    message_nums = torch.zeros_like(secret_tensor)
    for i in range(sigma):
        message_nums += message[:, i::sigma] * 2 ** (sigma - i - 1)
    secret_tensor = step * (message_nums + 0.5) - 1
...
def tensor_to_message_exp(secret_tensor, sigma):
    message = torch.zeros(size=(secret_tensor.shape[0], secret_tensor.shape[1] * sigma))
    step = 2 / 2 ** sigma
    secret_tensor = torch.clamp(secret_tensor, min=-1, max=1) + 1
    message_nums = secret_tensor / step
...
def message_to_tensor_exp_fun(message, sigma, delta):
    secret_tensor = torch.zeros(size=(message.shape[0], message.shape[1] // sigma))
    step = 2 / 2 ** sigma
    # step1 = 2 / exp(sigma)
    # step1 = 1 / ((exp(1)-2) ** (sigma-1))
    step1 = (exp(1)/2-1)**(sigma-1)
    print(step)
    print(step1)
    random_interval_size = step1 * delta
    message_nums = torch.zeros_like(secret_tensor)
    ...
    secret_tensor = step * (message_nums + 0.5) - 1
    secret_tensor = secret_tensor + (torch.rand_like(secret_tensor) * random_interval_size * 2 - random_interval_size)
    fun_tensor = torch.zeros_like(secret_tensor)
    for i in range(len(fun_tensor[0])):
        #fun_tensor[0][i] = (1 - (exp(1) - 2)**(sigma - 1))/(2**(sigma - 1))*sin((i+1)/2)    # total
        fun_tensor[0][i] = ((exp(1)/2)**(sigma - 1))*sin((i+1)/2)                           # part
    secret_tensor = secret_tensor + (fun_tensor * delta)
    return secret_tensor


def tensor_to_message_exp_fun(secret_tensor, sigma, delta):
    message = torch.zeros(size=(secret_tensor.shape[0], secret_tensor.shape[1] * sigma))
    step = 2 / 2 ** sigma
    fun_tensor = torch.zeros_like(secret_tensor)
#    for i in range(len(fun_tensor[0])):
...
def message_to_tensor_exp_fun(message, sigma, delta):
    ...
    fun_tensor = torch.zeros_like(secret_tensor)
    for i in range(len(fun_tensor[0])):
        fun_tensor[0][i] = ((exp(1)-2)*(sigma - 1))*sin((i+1)/2)/(2**(sigma - 1))
    secret_tensor = secret_tensor + (fun_tensor * delta)
    # for i in range(len(secret_tensor[0])):
    #     if secret_tensor[0][i] > 1:
    #         secret_tensor[0][i] = secret_tensor[0][i]-2
    #     elif secret_tensor[0][i] < -1:
    #         secret_tensor[0][i] = secret_tensor[0][i]+2
    return secret_tensor
...
def message_to_tensor_exp_fun(message, sigma, delta):
    ...
    fun_tensor = torch.zeros_like(secret_tensor)
    for i in range(len(fun_tensor[0])):
        # fun_tensor[0][i] = (2-exp(1)) + sin((i+1)/2)
        fun_tensor[0][i] = (2-exp(1))*sin((i+1)/2)/(2**(sigma - 1))
    secret_tensor = secret_tensor + fun_tensor * delta
    for i in range(len(secret_tensor[0])):
        if secret_tensor[0][i]>1:
            secret_tensor[0][i] = secret_tensor[0][i]-2
